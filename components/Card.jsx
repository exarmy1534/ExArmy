  ...
  return (
    <Link href={`/cars/${name}`} className="block"><div onMouseEnter={() => window.cursor?.enter()}
      ...
      <div className="relative">
        <div className="relative w-full h-56 bg-gradient-to-br from-yellow-50 to-amber-100">
          <Image
        ...
        {tag && (
          <span className="absolute top-4 left-4 bg-[#D97706] text-white text-xs px-3 py-1 rounded-full">
            {tag}
          </span>
        )}
      </div>
      ...
      <div className="p-5">
        ...
        <div className="mt-6 flex gap-3">
          <Link className=" w-52 flex items-center justify-center" href={`/contact?car=${encodeURIComponent(name)}`}> <button className="cursor-pointer active:bg-[#D97706] flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-[#FBBF24] to-[#D97706] text-white py-2.5 rounded-xl font-semibold hover:opacity-90 transition">
            <Calendar size={18} />
            Book Now
          </button></Link>
